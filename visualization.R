# turkey_project/visualization.R
...
plot_power_curve_enhanced = function(binned_data, raw_data = NULL, performance_metric = "ratio") {
...
plot_performance_histogram = function(df) {
  plot_ly(
    ...
    text = ~paste('Efficiency:', round(df$performance_ratio * 100, 1), '%'
    ),
    hoverinfo = 'x+y'
  ) %>%
    layout(
...
plot_timeseries_analysis = function(df) {
  daily_data = df %>%
    ...
    summarise(
      ...
      total_energy = sum(active_power_kw, na.rm = TRUE),
...
#' @param df Processed data frame
#' @param analysis_type Metric to use ('ratio', 'diff', or 'percent')
#' @return Plotly object
