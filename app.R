...
# Formatting helpers #################
# Generic number format: thousands separators + fixed decimals
fmt_num = function(x, digits = 1) {
  number_format(accuracy = 10^(-digits), big.mark = ",")(x)
}

# Percent format (expects 0–1 input)
fmt_pct = function(x, digits = 1) {
  label_percent(accuracy = 10^(-digits))(x)
}

# SI/compact format with units, e.g., 10.2 k, 3.5 M
fmt_si = function(x, digits = 1) {
  label_number_si(accuracy = 10^(-digits))(x)
}

# Energy-specific helper: choose best unit automatically from kWh → MWh → GWh
fmt_energy = function(kwh, digits = 1) {
  if (is.na(kwh)) return("--")
  if (abs(kwh) >= 1e6) {
    paste0(fmt_num(kwh / 1e6, digits), " GWh")
  } else if (abs(kwh) >= 1e3) {
    paste0(fmt_num(kwh / 1e3, digits), " MWh")
  } else {
    paste0(fmt_num(kwh, digits), " kWh")
  }
}

# Helper to render a big number with a smaller, lighter unit
# Use inside valueBox(value = ...)
fmt_with_unit = function(value_str, unit_str) {
  htmltools::tags$span(
    htmltools::HTML(
      paste0(
        '<span style="font-weight:700;">', value_str, '</span>',
        ' <span style="opacity:.85; font-weight:600;">', unit_str, '</span>'
      )
    )
  )
}
...
custom_css = tags$head(
  tags$style(HTML("
    /* Remove gap between Dashboard and Header */
    ...
    .main-sidebar .shiny-input-container > label {
      color: #000;
    }
  "))
)
    ...
    dashboardSidebar(
      ...
      box(
        ...
        sliderInput('power_range', 'Active Power (kW):',
                    min = 0, max = 1000, value = c(0, 1000), step = 25)
      ),
...
server = function(input, output, session) {
  ...
  observeEvent(input$data_file, {
    ...
    tryCatch({
      ...
      # Update wind speed and power sliders based on data
      if('wind_speed_ms' %in% colnames(processed)) {
        updateSliderInput(
          session,
          'wind_speed_range',
          min = floor(min(processed$wind_speed_ms, na.rm = TRUE)),
          max = ceiling(max(processed$wind_speed_ms, na.rm = TRUE)),
          value = c(
            min(processed$wind_speed_ms, na.rm = TRUE),
            max(processed$wind_speed_ms, na.rm = TRUE)
          )
        )
      }
      
      if('active_power_kw' %in% colnames(processed)) {
        updateSliderInput(
          session,
          'power_range',
          min = floor(min(processed$active_power_kw, na.rm = TRUE)),
          max = ceiling(max(processed$active_power_kw, na.rm = TRUE)),
          value = c(
            min(processed$active_power_kw, na.rm = TRUE),
            max(processed$active_power_kw, na.rm = TRUE)
          )
        )
      }
      
      # Auto-run analysis after data is processed
  ...
  ## Function to apply filters based on UI inputs ---------------
  apply_filters = function(data) {
    req(data)
    filtered = data
    
    # Filter by date range
    if('timestamp' %in% colnames(filtered)) {
      filtered = filtered %>%
        filter(
          timestamp >= as.POSIXct(input$date_range[1]),
          timestamp <= as.POSIXct(paste(input$date_range[2], "23:59:59"))
        )
    }
    
    # Filter by wind speed
    if('wind_speed_ms' %in% colnames(filtered)) {
      filtered = filtered %>%
        filter(
    ...
    if('active_power_kw' %in% colnames(filtered)) {
      filtered = filtered %>%
        filter(
  ...
  auto_run_analysis = function() {
    ...
    tryCatch({
      # For intial auto-run, use all processed data as filtered data
      rv$filtered_data = apply_filters(rv$processed_data)
      ...
      if(nrow(rv$filtered_data) > 0) {
        ...
        performance_metric = if(!is.null(input$performance_metric)) input$performance_metric else 'ratio'
        ...
        # Compute analysis results for valueBoxes
  ...
  # Update analysis when parameters applied
  observe({
    input$date_range
    input$wind_speed_range
    input$power_range
    
    if(rv$analysis_triggered && !is.null(rv$processed_data)) {
      isolate({
        rv$filtered_data = apply_filters(rv$processed_data)
        
        if(!is.null(rv$filtered_data) && nrow(rv$filtered_data) > 0) {
          rv$binned_data = create_power_curve_bins(
            df = rv$filtered_data,
            bin_width = input$bin_width,
            min_points = input$min_data_points,
  ...
  output$avg_power_box = renderValueBox({
    ...
    value_num = fmt_num(rv$analysis_results$avg_power, digits = 0)
    valueBox(
      value = fmt_with_unit(value_num, "kW"),
  ...
  output$avg_efficiency_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'avg_efficiency' %in% names(rv$analysis_results)) {
      return(valueBox('--', 
                      'Average Performance', 
    ...
    display_value = switch(    
      rv$analysis_results$performance_metric,    
      'ratio'   = fmt_pct(metric_value, digits = 1),
      'diff'    = fmt_with_unit(fmt_num(metric_value, 1), "kW"),
      'percent' = paste0(fmt_num(metric_value, 1), '%')
    )
  ...
  output$total_energy_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'total_energy' %in% names(rv$analysis_results)) {
      return(valueBox('--', 'Total Energy', icon = icon('charging-station'), color = 'yellow'))
    }
    
    valueBox(
      value = fmt_energy(rv$analysis_results$total_energy, digits = 2),
  ...
  output$data_points_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'data_points' %in% names(rv$analysis_results)) {
      return(valueBox('--', 'Data Points', icon = icon('database'), color = 'purple'))
    }
    
    valueBox(
      value = fmt_num(rv$analysis_results$data_points, digits = 0),
  ...
  output$avg_wind_speed_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'avg_wind_speed' %in% names(rv$analysis_results)) {
      return(valueBox('--', 'Avg Wind Speed', icon = icon('wind'), color = 'teal'))
    }
    
    value_num = fmt_num(rv$analysis_results$avg_wind_speed, digits = 2)
    valueBox(
      value = fmt_with_unit(value_num, ' m/s'),
  ...
  output$availability_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'availability' %in% names(rv$analysis_results)) {
      return(valueBox('--', 'Availability', icon = icon('check-circle'), color = 'red'))
    }
    
    valueBox(
      value = fmt_pct(rv$analysis_results$availability, digits = 1),
  ...
  output$max_power_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'max_power_output' %in% names(rv$analysis_results)) {
      return(valueBox('--', 'Max Power', icon = icon('arrow-up'), color = 'orange'))
    }
    
    value_num = fmt_num(rv$analysis_results$max_power_output, digits = 1)
    valueBox(
      value = fmt_with_unit(value_num, 'kW'),
  ...
  output$optimal_wind_box = renderValueBox({
    if (is.null(rv$analysis_results) || !'optimal_wind_speed' %in% names(rv$analysis_results)) {
      return(valueBox('--', 'Optimal Wind', icon = icon('tachometer-alt'), color = 'light-blue'))
    }
    
    value_num = fmt_num(rv$analysis_results$optimal_wind_speed, digits = 1)
    valueBox(
      value = fmt_with_unit(value_num,'m/s'),
  ...
  output$performance_histogram = renderPlotly({
    data_to_plot = if(!is.null(rv$filtered_data)) {
      rv$filtered_data
    } else {
      rv$processed_data
    }
    req(data_to_plot)
    plot_performance_histogram(data_to_plot)
  })
  ...
  output$wind_direction_plot = renderPlotly({
    ...
    tryCatch({
      plot_wind_direction_analysis(data_to_plot)
    }, error = function(e) {
      plotly_empty(type = 'scatter') %>%
        layout(
  ...
  output$timeseries_plot = renderPlotly({
    ...
    req(data_to_plot)
    plot_timeseries_analysis(data_to_plot)
  })
  ...
  output$scatter_comparison = renderPlotly({
    ...
    tryCatch({
      plot_scatter_comparison(data_to_plot)
    }, error = function(e) {
      plotly_empty(type = 'scatter') %>%
        layout(
          title = paste('Error in scatter comparison:', e$message),
          annotations = list(
            text = 'Please check if theoretical power data exists in your CSV file',
            xref = 'paper',
            yref = 'paper',
            x = 0.5,
            y = 0.5,
    ...
    if(is.null(data$binned_data) || !data$analysis_triggered) {
      return(plotly_empty(type = 'scatter') %>%
               layout(
                 ...
                 annotations = list(
                   text = 'Please run the analysis first or wait for auto-analysis',
                   xref = 'paper',
                   yref = 'paper',
                   x = 0.5,
                   y = 0.5,
  ...
  output$bin_summary_table = renderDT({
    ...
    # Add the performance metic column if exists
